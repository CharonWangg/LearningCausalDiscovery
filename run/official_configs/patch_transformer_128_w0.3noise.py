...
train_pipeline = [dict(type='TsAug', transforms=[dict(type='AddNoise', scale=0.3, normalize=False, seed=42)]),
                  dict(type='ToTensor')]
test_pipeline = [dict(type='TsAug', transforms=[dict(type='AddNoise', scale=0.3, normalize=False, seed=42)]),
                 dict(type='ToTensor')]
...
log = dict(
    ...
    work_dir='work_dir',
    exp_name='patch_transformer_128_0.3noise',
    logger_interval=50,
    monitor='val_auroc',
    logger=None,
