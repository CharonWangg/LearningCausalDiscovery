# model settings
num_classes = 2
loss = [dict(type='FocalLoss', loss_name='CrossEntropyLoss', loss_weight=1.0)]
...
model = dict(
    ...
    # auxiliary_head=None,
    evaluation = dict(metrics=[dict(type='TorchMetrics', metric_name='AveragePrecision', prob=True, num_classes=2),
                               dict(type='TorchMetrics', metric_name='AUROC', prob=True, pos_label=1),
                               dict(type='TorchMetrics', metric_name='Accuracy', prob=True),
                                 ])
)
...
log = dict(
    ...
    work_dir='work_dir',
    exp_name='patch_transformer_128_50ep_cosine_adamw_1e-3lr_256bs_0.05wd',
    logger_interval=50,
    monitor='val_auroc',
    logger=None,
