...
model = dict(
    ...
    backbone=dict(
        ...
        embedding=dict(
            ...
            mode="1d",
            input_norm=dict(
                type="BatchNorm1d", num_features=2, reshape_list=["b l c", "b c l"]
            ),
        ),
        hidden_size=128,
        num_layers=4,
    ...
    evaluation=dict(
        metrics=[
            dict(
                type="TorchMetrics",
                metric_name="Accuracy",  # step_reduction='mean',
                prob=True,
            ),
            dict(
                type="TorchMetrics",
...
optimization = dict(
    ...
    scheduler=dict(type="CosineAnnealing", interval="step", min_lr=1e-6, warmup=None),
)
